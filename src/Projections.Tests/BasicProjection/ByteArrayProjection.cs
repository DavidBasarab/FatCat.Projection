...
public class ByteArrayProjection
{
	...
	public void CanCopyTheArray()
	{
		...
		var otherArray = Array.CreateInstance(
											source.AByteArray.GetType().GetElementType(),
											source.AByteArray.Length
											);
		...
		otherArray.Should().BeEquivalentTo(source.AByteArray);
	}
	...
	public void TestingIsListForByteArray()
	{
		...
		TypeExtensions.IsArray(source.AByteArray.GetType()).Should().BeTrue();
	}
}
