...
public class MultiLevelObjectProjection
{
	...
	public void ObjectsAreNewlyCreated()
	{
		...
		ReferenceEquals(source, destination).Should().BeFalse();
	}
	...
	public void ProjectToAMultiLevelObjectDestination()
	{
		...
		destination.CreatedTime.Should().Be(source.CreatedTime);

		destination.Name.Should().Be(source.Name);

		destination.Level1.Time.Should().Be(source.Level1.Time);

		destination.Level1.TimesCreated.Should().Be(2);

		destination.Level1.DifferentSubObject.Number.Should().Be(source.Level1.DifferentSubObject.Number);

		destination
			.Level1.DifferentSubObject.UpdatedDate.Should()
			.Be(source.Level1.DifferentSubObject.UpdatedDate);

		destination.Level1.SubObject.Should().BeEquivalentTo(source.Level1.SubObject);
	}
	...
	public void ProjectToAMultiLevelObjectSameObject()
	{
		...
		result.Should().BeEquivalentTo(multiLevelObject);
	}
}