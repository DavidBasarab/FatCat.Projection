...
public class ProjectionToExistingObjectWithoutSameProperty
{
	...
	public void CanProjectToAClassWithoutDataInSourceAndDestinationDataStays()
	{
		...
		compareObject.MoreData.Should().Be(source.MoreData);

		compareObject.Id.Should().Be(destination.Id);

		compareObject.SomeData.Should().Be(source.SomeData);

		compareObject.NoOnSource.Should().Be(destination.NoOnSource);

		compareObject.SubClass.Should().NotBeNull();

		compareObject.SubClass.Data.Should().Be(destination.SubClass.Data);
	}
